...
class Status(object):
  ...
  def add(self, dat):
    ...
    j = json.dumps({'created_at': now.isoformat(), 'state': dat['state'], 'message': dat['message']})
  ...
  def load_states(self, dictionary):
    return dict([(k, map(json.loads,v) if k == 'states' else v) for k, v in dictionary.items()])
